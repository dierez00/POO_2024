...
class Coches:
    ...
    #En python el encapsulamiento tambien se llama visibilidad y por lo general define como seran los atributos y metodos, es decir
    #publicos o privados
    ...
    #Nota 1, para utilizar un atributo privado se tendria que hacer dento de un metodo publico
    def getPrivateAtribute(self):
...
class Camiones(Coches):
  ...
  def getInfo(self):
      print(f"Marca: {self.getMarca()} {self.getColor()}, numeros de plazas: {self.getPlazas()} \nModelo: {self.getModelo()} con una elocidad de {self.getVelocidad()} Km/h, un potencia de {self.getCaballaje()} hp, con {self.getEje()} ejes y una capacidad de carga {self.getCapacidadCarga()} metros cubicos")   
...
class Camionetas(Coches):
  ...
  def getInfo(self):
      print(f"Marca: {self.getMarca()} {self.getColor()}, numeros de plazas: {self.getPlazas()} \nModelo: {self.getModelo()} con una elocidad de {self.getVelocidad()} Km/h, un potencia de {self.getCaballaje()} hp, con la {self.getTraccion()} y es {self.getCerrada()}")   
